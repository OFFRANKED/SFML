...
, mAsteroid(textures.get(Textures::Asteroid))
, mShowExplosion(true)
{
	srand(time(NULL));
	...
	mAsteroid.setFrameSize(sf::Vector2i(64, 64));

	//Set the number of frame
	mExplosion.setNumFrames(16); 
	mAsteroid.setNumFrames(16);

	//Set the duration we want it to run for
	mExplosion.setDuration(sf::seconds(1));
	mAsteroid.setDuration(sf::seconds(1));
	mAsteroid.setRepeating(true);

	centerOrigin(mExplosion);
	centerOrigin(mAsteroid);
...
//Render either the normal aircraft or the explosion
	...
	if (!isAllied() && !isDestroyed()) {
		target.draw(mAsteroid, states);
	}
	else
#pragma endregion
...
void Aircraft::updateRollAnimation()
{
	if (Table[mType].hasRollAnimation)
	{
		...
		mSprite.setTextureRect(textureRect);
		//mAsteroid.setTextureRect(textureRect);
	}
}
#pragma endregion
...
void Aircraft::updateMovementPattern(sf::Time dt)
{
	...
	if (!directions.empty())
	{
		...
		float radians = toRadian(directions[mDirectionIndex].angle + num);
	...
	/*if (!directions.empty() && num == 2) {
		float radians = toRadian(directions[mDirectionIndex].angle + 500.f);
